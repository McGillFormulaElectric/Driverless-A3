...
- **A3.2** — transform cones all the way into `map`, through a **dynamic** `map -> base_link` published at 50 Hz by the professor. Because the car is moving, the same cone constellation traces circles in the map frame — that is the visual proof your TF chain works.
...
The professor runs the TF scenario + grader on the class Tailscale network. Every student joins the same tailnet so DDS discovery works between machines.
...
export A3_PROFESSOR_HOST=<professor tailnet host>  # e.g. professor.tail1234.ts.net
...
The professor node (`a3_professor tf_scenario_node`) does three things:
...
3. Publishes `/professor/sensor_cones` (`geometry_msgs/PoseArray`, `frame_id = "camera"`) at 20 Hz. Eight cones in a fixed constellation 5–15 m in front of the sensor.
...
**Goal:** subscribe to `/professor/sensor_cones`, transform every pose into `base_link` using the static camera mount, and publish the result on `/${GITHUB_USER}/cones_base_link` (`geometry_msgs/PoseArray`, `frame_id = "base_link"`).

Template: `ros2_ws/src/a3_student/a3_student/static_tf_node.py`. There's a `TODO` block inside `_on_cones`. Use `tf2_geometry_msgs.do_transform_pose(pose, tf)` — the `tf` is already looked up for you.
...
colcon build --symlink-install
source install/setup.bash
ros2 launch a3_student static.launch.py github_user:=$GITHUB_USER
```
...
**Deliverable for A3.1:** screenshot of `/professor/feedback`, committed to your branch as `submissions/a3_1_feedback.png`, plus your finished `static_tf_node.py`.
...
Template: `ros2_ws/src/a3_student/a3_student/dynamic_tf_node.py`. The code you write is *almost identical* to A3.1 — just ask tf2 for `map <- camera` at the message stamp. tf2 walks the tree `map <- base_link <- camera` for you.
...
colcon build --symlink-install
source install/setup.bash
ros2 launch a3_student dynamic.launch.py github_user:=$GITHUB_USER
```

**Deliverable for A3.2:** screenshot of `/professor/feedback`, plus a Foxglove screenshot showing the cones tracing circles in `map`, committed as `submissions/a3_2_feedback.png` and `submissions/a3_2_map.png`, plus your finished `dynamic_tf_node.py`.
...
3. Add a **3D** panel.
   ...
   - Add `/professor/sensor_cones` (drawn in the `camera` frame — will fly around with the car).
...
4. Add a **Raw Messages** panel on `/professor/feedback` to watch verdicts.
...
| ------------------------------ | --------------------------- | ------------ | -------- | ------------------------------------------ |
| `/tf`, `/tf_static`            | `tf2_msgs/TFMessage`        | —            | Professor | `base_link->camera` (static), `map->base_link` (50 Hz) |
| `/professor/sensor_cones`      | `geometry_msgs/PoseArray`   | `camera`     | Professor | 8 cones, 20 Hz                             |
...
├── ros2_ws/
│   └── src/
│       ├── a3_student/        # your template — this is where you write code
│       └── a3_professor/      # for reference; not run by students
└── README.md
```

## 8. Troubleshooting

- **`ros2 topic list` doesn't show `/professor/sensor_cones` or `/tf`.** DDS discovery isn't reaching the professor. Confirm `tailscale ping <professor-host>` works, that `A3_PROFESSOR_HOST` is set, and that `ROS_DOMAIN_ID` matches (`42`).
