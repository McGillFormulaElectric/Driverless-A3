...
if [ -n "${TS_AUTHKEY:-}" ]; then
  if command -v tailscale >/dev/null 2>&1; then
    echo "[a3] Joining MFE tailnet as '${GITHUB_USER:-a2-member}'..."
    tailscale up \
      --authkey="${TS_AUTHKEY}" \
...
NEIL_IP="${A3_NEIL_HOST:-100.127.203.84}"
